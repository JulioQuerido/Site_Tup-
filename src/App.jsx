...
const ChampionshipPortal = lazy(() => import('./pages/ChampionshipPortal'));
const MetricsDashboard = lazy(() => import('./pages/MetricsDashboard'));
const StandBooking = lazy(() => import('./pages/StandBooking'));
...
function App() {
  ...
  useEffect(() => {
    if (!isProtectedRoute(currentRoute)) return;

    // Guard apenas de UX — impede navegacao acidental. A protecao real dos
  ...
  const suppressNavFooter = STANDALONE_ROUTES.includes(currentRoute);

  const renderContent = () => {
    switch (currentRoute) {
      case '#/campeonato-1v1':
        return <ChampionshipPortal />;
      case '#/painel-metricas':
        return <AdminLayout currentRoute={currentRoute}><MetricsDashboard /></AdminLayout>;
      case '#/agendamento-estande':
        return <StandBooking />;
      case '#/login':
        return <AdminLogin />;
      case '#/alterar-senha':
        ...
        return (
          <main className="flex-grow">
              ...
              <div className="relative z-10 max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center w-full">
                <div className="flex flex-col gap-6 text-center md:text-left">
                  ...
                  <div className="flex flex-wrap gap-4 justify-center md:justify-start mt-4">
                    ...
                    <a 
                      href="#/campeonato-1v1"
                      className="bg-accent text-[#001e2c] font-bold px-6 py-3 rounded-lg hover:shadow-[0_0_20px_rgba(0,191,255,0.8)] transition-all font-label-sm text-sm uppercase tracking-wider inline-flex items-center gap-2"
                    >
                      Torneio 1v1
                      <span className="material-symbols-outlined text-sm font-bold" style={{ fontVariationSettings: "'FILL' 1" }}>emoji_events</span>
                    </a>
                  </div>
                </div>
            ...
            >
              ...
              <Suspense fallback={<div className="h-80 rounded-xl bg-surface-container/40 animate-pulse" />}>
                <AudienceChart />
              </Suspense>
            </section>
                ...
                <div className="p-6 md:p-12">
                  ...
                  <div className="relative border-l border-outline-variant ml-4 md:ml-6 space-y-12">
                    ...
                    <div className="relative pl-8 md:pl-12">
                      ...
                      {/* Action buttons to go to booking and admin dashboard */}
                      <div className="flex flex-wrap gap-3 mt-3">
                        <a 
                          href="#/agendamento-estande" 
                          className="bg-primary/20 text-primary border border-primary/40 hover:bg-primary/30 px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-wider transition-all inline-flex items-center gap-1.5 shadow-[0_0_10px_rgba(143,214,255,0.15)]"
                        >
                        ...
                        <a 
                          ...
                          className="bg-secondary/20 text-secondary border border-secondary/40 hover:bg-secondary/30 px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-wider transition-all inline-flex items-center gap-1.5"
                        >
            ...
            >
              <div>
                <h2 className="font-display-lg text-4xl md:text-5xl text-on-surface mb-4 font-bold uppercase tracking-widest">
                  Iniciar <br/>
                  <span className="text-primary text-glow font-bold">Protocolo</span>
                </h2>
  ...
  return (
    // `overflow-x-clip` e nao `overflow-x-hidden`: `overflow: hidden` cria um
    // contexto de rolagem e quebra o `position: sticky` do Header, que passa a
    // grudar neste container (do tamanho da pagina) em vez da viewport.
    // `overflow: clip` corta o transbordo horizontal sem esse efeito colateral.
    <div className="font-body-md bg-background min-h-screen flex flex-col relative grid-bg overflow-x-clip text-on-surface">

      {/* Aviso de configuracao ausente — evita que o modo offline passe despercebido */}
      {!isSupabaseConfigured && (
        <div role="alert" className="bg-error-container/30 border-b border-error/40 text-error px-4 py-2 text-center text-xs font-semibold">
          Banco de dados não configurado — exibindo dados locais de demonstração.
      ...
      {/* Header */}
      {!suppressNavFooter && (
        <Header activeSection={activeSection} scrollToSection={scrollToSection} currentRoute={currentRoute} session={session} />
      )}
